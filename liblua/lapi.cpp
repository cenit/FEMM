...
LUA_API int lua_lessthan(lua_State* L, int index1, int index2)
{
  ...
  if (o1 == NULL || o2 == NULL)
    ...
    return luaV_lessthan(L, o1, o2, L->top);
}
...
{
  StkId o = luaA_indexAcceptable(L, index);
  return (o == NULL || ttype(o) != LUA_TUSERDATA) ? NULL : tsvalue(o)->u.d.value;
}
...
LUA_API int lua_getref(lua_State* L, int ref)
{
  if (ref == LUA_REFNIL)
    ttype(L->top) = LUA_TNIL;
  else if (0 <= ref && ref < L->refSize && (L->refArray[ref].st == LOCK || L->refArray[ref].st == HOLD))
...
LUA_API int lua_getn(lua_State* L, int index)
{
  ...
  if (ttype(value) == LUA_TNUMBER)
    ...
    Node* n = h->node;
    while (i--) {
      if (ttype(key(n)) == LUA_TNUMBER && ttype(val(n)) != LUA_TNIL && Re(nvalue(key(n))) > Re(max))
        max = nvalue(key(n));
      n++;
    }
    return (int)Re(max);
