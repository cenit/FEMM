...
{
  lua_pushnumber(L, lua_copytagmethods(L, luaL_check_int(L, 1), luaL_check_int(L, 2)));
  return 1;
}
...
{
  static const char* const errornames[] = { "ok", "run-time error", "file error", "syntax error",
  ...
  while (l < u) { /* for tail recursion */
    ...
    for (;;) { /* invariant: a[l..i] <= P <= a[j..u] */
      ...
      while (lua_rawgeti(L, 1, ++i), sort_comp(L, -1, -2)) {
        if (i > u)
          lua_error(L, "invalid order function for sorting");
