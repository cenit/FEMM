...
void luaV_strconc(lua_State* L, int total, StkId top)
{
  do {
    ...
    if (tostring(L, top - 2) || tostring(L, top - 1)) {
      ...
      if (tl > MAX_SIZET)
        lua_error(L, "string size overflow");
...
StkId luaV_execute(lua_State* L, const Closure* cl, StkId base)
{
  ...
  for (;;) {
    ...
    case OP_NOT: {
      ttype(top - 1) = (ttype(top - 1) == LUA_TNIL) ? LUA_TNUMBER : LUA_TNIL;
      nvalue(top - 1) = 1;
      break;
    }
    case OP_JMPNE: {
      top -= 2;
      if (!luaO_equalObj(top, top + 1))
        dojump(pc, i);
      break;
    }
    case OP_JMPEQ: {
    ...
    case OP_JMPLT: {
      top -= 2;
      if (luaV_lessthan(L, top, top + 1, top + 2))
        dojump(pc, i);
      break;
    }
    case OP_JMPLE: { /* a <= b  ===  !(b<a) */
      top -= 2;
      if (!luaV_lessthan(L, top + 1, top, top + 2))
        dojump(pc, i);
      break;
    }
    ...
    case OP_FORPREP: {
      ...
      if (nvalue(top - 1) > 0 ? nvalue(top - 3) > nvalue(top - 2) : nvalue(top - 3) < nvalue(top - 2)) { /* `empty' loop? */
