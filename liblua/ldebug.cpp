...
LUA_API int lua_getstack(lua_State* L, int level, lua_Debug* ar)
{
  ...
  if (f == NULL)
    return 0; /* there is no such level */
  else {
    ar->_func = f;
    return 1;
  }
...
LUA_API const char* lua_getlocal(lua_State* L, const lua_Debug* ar, int n)
{
  ...
  if (!fp)
    return NULL; /* `f' is not a Lua function? */
...
LUA_API const char* lua_setlocal(lua_State* L, const lua_Debug* ar, int n)
{
  ...
  name = luaF_getlocalname(fp, n, currentpc(f));
  if (!name || name[0] == '(')
    return NULL; /* `(' starts private locals */
  *((f + 1) + (n - 1)) = *L->top;
  return name;
}
...
static const char* travglobals(lua_State* L, const TObject* o)
{
  ...
  int i;
  for (i = 0; i < g->size; i++) {
    if (luaO_equalObj(o, val(node(g, i))) && ttype(key(node(g, i))) == LUA_TSTRING)
...
static const char* getfuncname(lua_State* L, StkId f, const char** name)
{
  ...
  if (!isLmark(func))
    ...
    Instruction i;
    if (pc == -1)
      return NULL; /* function is not activated */
    i = p->code[pc];
    switch (GET_OPCODE(i)) {
    case OP_CALL:
    case OP_TAILCALL:
