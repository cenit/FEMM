...
*/
	...
	if (IntegralType==2) // (Time-Average) Stored Energy
	{
		...
		for(k=0;k<thisDoc->agelist[i].nn;k++)
		{
			...
			if (n!=0)
				W+= (thisDoc->agelist[i].brs[k]*thisDoc->agelist[i].brs[k] +
					thisDoc->agelist[i].brc[k]*thisDoc->agelist[i].brc[k] +
					thisDoc->agelist[i].bts[k]*thisDoc->agelist[i].bts[k] +
					thisDoc->agelist[i].btc[k]*thisDoc->agelist[i].btc[k])*dr;
			else
