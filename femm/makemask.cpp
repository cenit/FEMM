...
BOOL CFemmviewDoc::MakeMask()
{
  ...
  for (i = 0; i < blockproplist.GetSize(); i++) {
    ...
    k = 0;
    if ((blockproplist[i].mu_x != 1) || (blockproplist[i].mu_y != 1))
      k = 1;
    if (blockproplist[i].BHpoints != 0)
      k = 1;
    if (blockproplist[i].LamType != 0)
      k = 1;
    if (blockproplist[i].H_c != 0)
      k = 1;
    if ((blockproplist[i].Jr != 0) || (blockproplist[i].Ji != 0))
      k = 1;
    if (blockproplist[i].Cduct != 0)
      k = 1;
    if ((blockproplist[i].Theta_hn != 0) || (blockproplist[i].Theta_hx != 0) || (blockproplist[i].Theta_hy != 0))
      k = 1;
    matflag[i] = k;
  }
