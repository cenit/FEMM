    ...
    CRgn* pRegion)
{
  ...
  BOOL bClipping = layout.properties.bAskClipping ? LikesClipping(layout) : layout.properties.bCachedLikesClipping;
...
BOOL CResizableLayout::LikesClipping(const CResizableLayout::LayoutInfo& layout)
{
  ...
  if (layout.sWndClass == TOOLBARCLASSNAME && (style & TBSTYLE_TRANSPARENT))
    ...
    case BS_OWNERDRAW:
      ...
      ::GetWindowRect(layout.hWnd, &rect);
      if (HTTRANSPARENT == ::SendMessage(layout.hWnd, WM_NCHITTEST, 0, MAKELPARAM(rect.left, rect.top)))
        return FALSE;
      break;
    }
    return TRUE;
    ...
    const CRect& rectParent, CRect& rectChild, UINT& uFlags)
{
  ...
  BOOL bRefresh = layout.properties.bAskRefresh ? NeedsRefresh(layout, rectChild, rectNew) : layout.properties.bCachedNeedsRefresh;
