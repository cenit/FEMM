...
BOOL CFemmeDocCore::Cuthill()
{
  ...
  for (i = 0; i < NumAGEs; i++) {
    for (k = 0; k <= agelist[i].totalArcElements; k++) {
      agelist[i].node[k].n0 = newnum[agelist[i].node[k].n0];
      agelist[i].node[k].n1 = newnum[agelist[i].node[k].n1];
      agelist[i].node[k].n2 = newnum[agelist[i].node[k].n2];
      agelist[i].node[k].n3 = newnum[agelist[i].node[k].n3];
    }
  }
