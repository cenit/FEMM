...
enum insertvertexresult { SUCCESSFULVERTEX,
  ENCROACHINGVERTEX,
  VIOLATINGVERTEX,
  DUPLICATEVERTEX };
...
enum finddirectionresult { WITHIN,
  LEFTCOLLINEAR,
  RIGHTCOLLINEAR };
...
#define decode(ptr, otri)                                  \
  ...
  (otri).tri = (triangle*)((uintptr_t)(ptr) ^ (uintptr_t)(otri).orient)
...
#define infect(otri) \
  (otri).tri[6] = (triangle)((uintptr_t)(otri).tri[6] | (uintptr_t)2l)
...
#define sdecode(sptr, osub)                                   \
  ...
  (osub).ss = (subseg*)((uintptr_t)(sptr) & ~(uintptr_t)3l)
...

{
  ...
  pool->nextitem = (VOID*)(alignptr + (uintptr_t)pool->alignbytes - (alignptr % (uintptr_t)pool->alignbytes));
...

{
  ...
  pool->firstblock = (VOID**)
      trimalloc(pool->itemsfirstblock * pool->itembytes + (int)sizeof(VOID*) + pool->alignbytes);
...

{
  ...
  pool->pathitem = (VOID*)(alignptr + (uintptr_t)pool->alignbytes - (alignptr % (uintptr_t)pool->alignbytes));
...

{
  ...
  m->dummytri = (triangle*)(alignptr + (uintptr_t)m->triangles.alignbytes - (alignptr % (uintptr_t)m->triangles.alignbytes));
  ...
  if (b->usesegments) {
    ...
    m->dummysub = (subseg*)(alignptr + (uintptr_t)m->subsegs.alignbytes - (alignptr % (uintptr_t)m->subsegs.alignbytes));
...

{
  ...
  foundvertex = (char*)(alignptr + (uintptr_t)m->vertices.alignbytes - (alignptr % (uintptr_t)m->vertices.alignbytes));
...
#else /* not ANSI_DECLARATORS */
    int
    fast_expansion_sum_zeroelim(elen, e, flen, f, h) /* h cannot be e or f. */
int elen;
REAL* e;
int flen;
REAL* f;
...
#else /* not ANSI_DECLARATORS */
    int
    scale_expansion_zeroelim(elen, e, b, h) /* e and h cannot be the same. */
int elen;
REAL* e;
REAL b;
REAL* h;
...
#ifdef ANSI_DECLARATORS
    REAL
    incircleadapt(vertex pa, vertex pb, vertex pc, vertex pd, REAL permanent)
#else /* not ANSI_DECLARATORS */
    REAL
...
#ifdef ANSI_DECLARATORS
    REAL
    incircle(struct mesh* m, struct behavior* b,
        vertex pa, vertex pb, vertex pc, vertex pd)
#else /* not ANSI_DECLARATORS */
    REAL
    incircle(m, b, pa, pb, pc, pd)
struct mesh* m;
struct behavior* b;
vertex pa;
vertex pb;
...
#ifdef ANSI_DECLARATORS
    REAL
    orient3dadapt(vertex pa, vertex pb, vertex pc, vertex pd,
        REAL aheight, REAL bheight, REAL cheight, REAL dheight,
        REAL permanent)
#else /* not ANSI_DECLARATORS */
    REAL
    orient3dadapt(pa, pb, pc, pd,
...
#ifdef ANSI_DECLARATORS
    REAL
    orient3d(struct mesh* m, struct behavior* b,
        vertex pa, vertex pb, vertex pc, vertex pd,
        REAL aheight, REAL bheight, REAL cheight, REAL dheight)
#else /* not ANSI_DECLARATORS */
    REAL
    orient3d(m, b, pa, pb, pc, pd, aheight, bheight, cheight, dheight)
struct mesh* m;
struct behavior* b;
vertex pa;
vertex pb;
...

{
  ...
  while (triangleloop.tri != (triangle*)NULL) {
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      shouldbedelaunay = (oppotri.tri != m->dummytri) && !deadtri(oppotri.tri) && (triangleloop.tri < oppotri.tri) && (triorg != m->infvertex1) && (triorg != m->infvertex2) && (triorg != m->infvertex3) && (tridest != m->infvertex1) && (tridest != m->infvertex2) && (tridest != m->infvertex3) && (triapex != m->infvertex1) && (triapex != m->infvertex2) && (triapex != m->infvertex3) && (oppoapex != m->infvertex1) && (oppoapex != m->infvertex2) && (oppoapex != m->infvertex3);
...

{
  ...
  if (neighbortri.tri != m->dummytri) {
    ...
    if (dotproduct < 0.0) {
      if (b->conformdel || (dotproduct * dotproduct >= (2.0 * b->goodangle - 1.0) * (2.0 * b->goodangle - 1.0) * ((eorg[0] - eapex[0]) * (eorg[0] - eapex[0]) + (eorg[1] - eapex[1]) * (eorg[1] - eapex[1])) * ((edest[0] - eapex[0]) * (edest[0] - eapex[0]) + (edest[1] - eapex[1]) * (edest[1] - eapex[1])))) {
        encroached = 1;
      }
    }
  }
...

{
  ...
  while (totalsamplesleft > 0) {
    ...
    firsttri = (char*)(alignptr + (uintptr_t)m->triangles.alignbytes - (alignptr % (uintptr_t)m->triangles.alignbytes));
    ...
    while ((encloop != (struct badsubseg*)NULL) && (m->steinerleft != 0)) {
      ...
      if (!deadsubseg(currentenc.ss) && (eorg == encloop->subsegorg) && (edest == encloop->subsegdest)) {
        ...
        if (!b->conformdel && !acuteorg && !acutedest) {
          apex(enctri, eapex);
          while ((vertextype(eapex) == FREEVERTEX) && ((eorg[0] - eapex[0]) * (edest[0] - eapex[0]) + (eorg[1] - eapex[1]) * (edest[1] - eapex[1]) < 0.0)) {
        ...
        if (acuteorg || acutedest) {
          segmentlength = sqrt((edest[0] - eorg[0]) * (edest[0] - eorg[0]) + (edest[1] - eorg[1]) * (edest[1] - eorg[1]));
...

{
  ...
  if ((m->eextras > 0) && (*triangleattriblist == (REAL*)NULL)) {
    *triangleattriblist = (REAL*)trimalloc((int)(m->triangles.items * m->eextras * sizeof(REAL)));
  }
  tlist = *trianglelist;
...

{
  ...
  if (*vpointattriblist == (REAL*)NULL) {
    *vpointattriblist = (REAL*)trimalloc((int)(m->triangles.items * m->nextras * sizeof(REAL)));
  }
